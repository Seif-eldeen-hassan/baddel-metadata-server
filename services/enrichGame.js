...
async function _lookupOnly(item) {
    ...
    let screenshots = [];
    let coverMeta   = { width: null, height: null };
    let heroMeta    = { width: null, height: null };
    ...
    if (platform === 'steam' && steamPythonData) {
        let steamCover = await getValidSteamCover(steamPythonData.poster);

        let steamHero = steamPythonData.background;
        let isSteamHeroBad = false;

        const badHeroPatterns = ['storepage', 'page_bg_raw', 'page_bg_generated'];
        if (steamHero && badHeroPatterns.some(pattern => steamHero.includes(pattern))) {
            isSteamHeroBad = true;
        }
        ...
        if (isSteamHeroBad) {
            heroSource = sgdbData.hero?.url || igdbData?.images?.artworks?.[0]?.url || steamHero;
        } else {
...
async function _saveRawUrls(resolved) {
    ...
    const isRichData = (data) => {
        ...
        return !!data.developer || (data.description && data.description.length > 20);
    };
    ...
    if (storeData) {
        ...
        const storePromises = [
            ...
            ratingScore > 0 ? upsertRating(gameId, { score: ratingScore, max_score: ratingMax, source: ratingSource }) : Promise.resolve(),
